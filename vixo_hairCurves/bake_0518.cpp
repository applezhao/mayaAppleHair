...
{
	//cout<<"test dirty:"<<plugBeingDirtied.info().asChar()<<endl;
	//cout<<plugBeingDirtied.partialName().substring(plugBeingDirtied.partialName().length()-2,plugBeingDirtied.partialName().length()).asChar()<<endl;
	//if(plugBeingDirtied.partialName()
	MObject thisNode=thisMObject();
	MPlug arrayOutCurve(thisNode,this->outCurves);
	...
	if(arrayOutCurve.numElements()>0)
	{
		...
		{
			affectedPlugs.append(arrayOutCurve[0].child(this->outCurveShape));
		}
	}
	return MS::kSuccess;
}
