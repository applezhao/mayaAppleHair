...
void vixo_hairRender::renderTriangle()
{
	...
	//for(int i=0;i<clumpSet.size();i++)
	//{
	//	cout<<"wei"<<forClumpUV[i].w0<<" "<<forClumpUV[i].w1<<" "<<forClumpUV[i].w2<<endl;
	//}
	...
	{
		cout<<"diff:"<<renderData.hairShapeSpan[j].x<<" "<<renderData.hairShapeSpan[j].y<<" "<<renderData.hairShapeSpan[j].z<<endl;
	}
	for(int i=0;i<clumpSet.size();i++)
	{
		cout<<"clump-------"<<i<<endl;
		for(int j=0;j<span;j++)
		{
			cout<<clumpData.hairShapeSpan[i*span+j].x<<" "<<clumpData.hairShapeSpan[i*span+j].y<<" "<<clumpData.hairShapeSpan[i*span+j].z<<endl;
		}
	}*/
	/*
	for(int i=0;i<clumpSet.size();i++)
	{
		cout<<"clump "<<i<<endl;
		for(int j=0;j<hairParameters.segment+3;j++)
		{
			cout<<clumpData.hairShapeSegment[(i*(hairParameters.segment+3)+j)*3]<<" "<<clumpData.hairShapeSegment[(i*(hairParameters.segment+3)+j)*3+1]<<" "<<clumpData.hairShapeSegment[(i*(hairParameters.segment+3)+j)*3+2]<<endl;
		}
		cout<<"clump end"<<endl;
	}
	*/
...
void vixo_hairRender::deformPolarIncliCurl(vector<float>& curlParaHair,vector<float>& curlParaClump,int clumpSetSize)
{
	...
	for(int i=0;i<triangleHairs.hairNum;i++)
	{
		...
		for(int j=0;j<span;j++)
		{
			...
			renderData.hairShapeSpan[i*span+j]=renderData.hairShapeSpan[i*span+j].rotate(normal,polar.hairValue[i]*M_PI);
	...
	for(int i=0;i<clumpSetSize;i++)
	{
		...
		for(int j=0;j<span;j++)
		{
			...
			clumpData.hairShapeSpan[i*span+j]=clumpData.hairShapeSpan[i*span+j].rotate(normal,polar.clumpValue[i]*M_PI);
			clumpData.hairShapeSpan[i*span+j]=clumpData.hairShapeSpan[i*span+j].rotate(binormal,inclination.clumpValue[i]*M_PI/2);
			clumpData.hairShapeSpan[i*span+j]=clumpData.hairShapeSpan[i*span+j].rotate(binormal,curlParaClump[i]*M_PI/2);
